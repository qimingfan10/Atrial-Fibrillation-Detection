...
class MobileNetV2FeatureExtractor(nn.Module):
    def __init__(self):
        ...
        # mobilenet_v2.load_state_dict(torch.load('mobilenet_v2-b0353104.pth')) # 从本地文件加载
...
# 加载数据集
img_dir = "./shaoxing30sthree"
dataset = datasets.ImageFolder(img_dir, transform=transform, loader=my_loader)

# 获取AF、N和Other类别的索引
class_indices = {'AF': [], 'N': [], 'Other': []}
for idx, (_, label) in enumerate(tqdm(dataset, desc="Loading dataset")):
...
num_Other = len(class_indices['Other'])
total_samples = num_AF + num_N + num_Other
af_ratio = num_AF / total_samples
n_ratio = num_N / total_samples
other_ratio = num_Other / total_samples

print(f"AF samples: {num_AF}, N samples: {num_N}, Other samples: {num_Other}, AF ratio: {af_ratio:.2%}, N ratio: {n_ratio:.2%}, Other ratio: {other_ratio:.2%}")
...
# 打印数据集的大小
print(f"Train set size: {len(train_data)} (AF: {af_ratio:.2%}, N: {n_ratio:.2%}, Other: {other_ratio:.2%})")
